...
<div class="mc_merge_var">
		...
		switch ($var['field_type']) {
			case 'date': 
				$html .= '
	<input type="text" size="18" placeholder="'.esc_attr($var['name']).'" data-format="'.esc_attr($var['dateformat']).'" name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="date-pick mc_input"/>';
				break;
			case 'radio':
	...
	<ul class="mc_list">';
		...
		<option value="'.esc_attr($value).'"'.selected($value, $var['default'], false).'>'.esc_html($value).'</option>';
			...
			case 'birthday':
				$html .= '
	<input type="text" size="18" placeholder="'.esc_attr($var['name']).'" data-format="'.esc_attr($var['dateformat']).'" name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="birthdate-pick mc_input"/>';
				break;
			case 'birthday-old':
				$days = range(1, 31);
